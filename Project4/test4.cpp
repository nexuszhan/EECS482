...
#include "fs_client.h"
// multiple write
// multi user access other user's data
// delete twice, delete non-exist
...
int main(int argc, char *argv[])
{
    ...
    status = fs_create("user2", "/dir/file3", 'd');

    status = fs_writeblock("user1", "/dir/file1", 0, writedata1);
    assert(!status);

    status = fs_writeblock("user2", "/dir/file1", 0, writedata2);

    status = fs_readblock("user1", "/dir/file1", 0, readdata);
    assert(!status);

    status = fs_readblock("user2", "/dir/file1", 0, readdata);
